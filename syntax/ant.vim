...
"syn region  xmlString contained start=+"+ end=+"+ contains=xmlEntity,expression display
"syn region  xmlString contained start=+'+ end=+'+ contains=xmlEntity,expression display

syn match   expression "\${[^}]*}" containedin=xmlString display

hi def link antElement Statement
hi def link expression Special
